...
if(loggedin()){
	...
	$(document).ready(function(){
		var form_open = 0;
		var form_html = $("#start-new-pm").html();
		function open_form(){
			$("#start-new-pm").html(form_html);
			$("#start-new-pm").animate({marginTop:"30px"}).animate({backgroundColor:"#C9C9C9"}).animate({padding:"4px"}).css("text-align", "center")
			.animate({marginLeft:"13%"}).animate({width:"30%"}).animate({height:"400px"});
			setTimeout(function(){
		...
		$("#close-form").click(function(){
			...
			setTimeout(function(){
			$("#snm-text").fadeIn();
			$("#start-new-pm").html("Start New Message").animate({marginTop:"-4px"}).css({backgroundColor:"transparent"});
	...
	<br>
	<div class = 'close-spm-form' id = 'close-form' style = "">x</div>
		...
		<div id ='start-new-pm' style ="position:absolute;border-radius:3px;display:inline;">
			...
			<div id = 'start-pm-form' style = "display:none;">
				<form action = "" method = "POST"><br>
					...
					To:<input name = "pm_to" type = "text" id = "desto" class = "loggedout-form-fields" style = "box-shadow:none;height:25px;width:80%;margin-left:-2px;font-size:70%;letter-spacing:1px;" placeholder = "Usernames or Groups, separated by commas"><br><br>
					<div id = "pred_results"></div>
					Message:<textarea id = 'pm_body_' name = "pm_body" class= "textarea-type1" style = "border-radius:3px;width:90%;font-size:80%;letter-spacing:1px;" placeholder="Your message..."></textarea><br><br>
	...
	/*$(document).ready(function(){
		...
		$("#desto").keyup(function(){
			alert($("#desto").val());
			var str = $(this).val();
			if(str.length>0){
				$.post("/projects/buzzzap1/ajax_script.php", {ajax_search:"true", table:"users", column:"user_username", str:str}, function(result){
	...
	<div id = "all-messages">
		...
		$get_rel_messages = $db->prepare("SELECT pm_id FROM pm_members WHERE user_id = :user_ident AND visible = 1");
		...
		if($get_rel_messages->rowCount()>0){
			while($row = $get_rel_messages->fetch(PDO::FETCH_ASSOC)){
				$pm_id = $row['pm_id'];
				$get_members = $db->prepare("SELECT user_id FROM pm_members WHERE pm_id = :pm_id");
